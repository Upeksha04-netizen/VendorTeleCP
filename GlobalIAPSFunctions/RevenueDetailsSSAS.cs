...
{
    public static class RevenueDetailsSSAS
    {
        [FunctionName("RevenueDetailsSSAS")]
            ...
            ILogger log)
        {
            ...
            crmManager.InsertSSASCubeData("[dbo].[KPIResults_RevenueForecast]", StartTime);
            crmManager.InsertSSASCubeData("[dbo].[KPIResults_CustomerAdds]",StartTime);
            crmManager.InsertSSASCubeData("[dbo].[KPIResults_CloudRevenue]", StartTime);
            crmManager.InsertSSASCubeData("[dbo].[KPIResults_AzureConsumedRevenue]", StartTime);
            crmManager.InsertSSASCubeData("[dbo].[KPIResults_NPSA]", StartTime);
            crmManager.InsertSSASCubeData("[dbo].[KPIResults_Renewals]", StartTime);
            crmManager.InsertSSASCubeData("[dbo].[RevSumMapping]", StartTime);
            //crmManager.InsertSSASCubeData("[dbo].[Calendar]", StartTime);
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation(" Update Lead stage method is successfully executed");
                }
            });
        }
    }
