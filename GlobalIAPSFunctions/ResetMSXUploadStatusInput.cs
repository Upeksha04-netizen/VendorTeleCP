...
{
    public static class ResetMSXUploadStatusInput
    {
        [FunctionName("ResetMSXUploadStatusInput")]
            ...
            ILogger log)
        {
            ...
            crmManager.InsertResetMSXleadIntoTable();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("Upload to ResetMSXUploadStatusInput is successfully executed");
                }
            });
        }
    }
