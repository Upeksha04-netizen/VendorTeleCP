...
{
    public static class ResetMSXUploadStatusOutput
    {
        [FunctionName("ResetMSXUploadStatusOutput")]
            ...
            ILogger log)
        {
            ...
            crmManager.RetrieveResetMSXStatusFromTable();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("Upload to ResetMSXUploadStatusOutput is successfully executed");
                }
            });
        }
    }
