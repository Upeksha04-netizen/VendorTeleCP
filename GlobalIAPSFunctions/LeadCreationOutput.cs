...
{
    public static class LeadCreationOutput
    {
        [FunctionName("LeadCreationOutput")]
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("Auto Lead Creation Output function is successfully executed");
                }
            });
        }
    }
