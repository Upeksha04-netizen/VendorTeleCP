...
{
    public static class LeadCreationInput
    {
        [FunctionName("LeadCreationInput")]
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("Auto Lead Creation Input function is successfully executed");
                }
            });
        }
    }
