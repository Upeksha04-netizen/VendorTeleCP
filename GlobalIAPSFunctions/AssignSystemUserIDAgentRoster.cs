...
{
    public static class AssignSystemUserIDAgentRoster
    {
        [FunctionName("AssignSystemUserIDAgentRoster")]
            ...
            ILogger log)
        {
            ...
            crmManager = new Manager(Instancename, ClientId, ClientSecret, MSXClientId, MSXClientSecret, sqlConnection, ssasServer, blobConnection, vtcpManagedIdentity, log);
            crmManager.UpdateUserIdAgentRoster();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation(" Update AgentRosterUserID method is successfully executed");
                }
            });
        }
    }
