...
{
    public static class UpdateAutoNumberExistingLeads
    {
            [FunctionName("UpdateAutoNumberExistingLeads")]
            ...
            ILogger log)
        {
            ...
            crmManager = new LeadCleanupFunctions(Instancename, ClientId, ClientSecret, MSXClientId, MSXClientSecret, sqlConnection, ssasServer, blobConnection,vtcpManagedIdentity, log);
                crmManager.UpdateAutoNumberLeads();
            ...
            public static async Task Dispose(ILogger log)
            {
                await Task.Run(() =>
                {
                    for (int i = 0; i < 100; i++)
                    {
                        log.LogInformation("Update Auto Number method is successfully executed");
                    }
                });
            }
        }
