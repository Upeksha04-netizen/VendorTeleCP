...
{
    public static class RenewalLeadsCheckInput
    {
        [FunctionName("RenewalLeadsCheckInput")]
            ...
            ILogger log)
        {
            ...
            crmManager.InsertRenewalLeadsDtailsIntoTable();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation(" Insert RenewalLeadsCheckInput method is successfully executed");
                }
            });
        }
    }
