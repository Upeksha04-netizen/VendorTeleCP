...
{
    public static class SMBVT_Leads
    {
        [FunctionName("SMBVT_Leads")]
            ...
            ILogger log)
        {
            ...
            crmManager.InsertSSASCubeData("[dbo].[SMBVT_Leads]",StartTime);
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation(" Update SMBVT_Leads method is successfully executed");
                }
            });
        }
    }
