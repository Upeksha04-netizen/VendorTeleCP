...
{
    public static class MSXIntegrationOutput
    {
        [FunctionName("MSXIntegrationOutput")]
            ...
            ILogger log)
        {
            ...
            crmManager.RetrieveMSXRecordsFromTable();
            //log.LogInformation("Update MSX method is successfully executed");
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation(" Update MSX Data method is successfully executed");
                }
            });
        }
    }
