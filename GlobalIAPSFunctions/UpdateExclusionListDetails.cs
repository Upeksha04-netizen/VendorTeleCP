...
{
    public static class UpdateExclusionListDetails
    {
        [FunctionName("UpdateExclusionListDetails")]
            ...
            ILogger log)
        {
            ...
            crmManager.UpdateIsExclusionListField();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("Update ExclusionList method is successfully executed");
                }
            });
        }
    }
