...
{
    public static class LeadValidationExpiredCheck
    {
        [FunctionName("LeadValidationExpiredCheck")]
            ...
            ILogger log)
        {
            ...
            crmManager.LeadValidationExpiredCheck();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("LeadValidationExpiredCheck method is successfully executed");
                }
            });
        }
    }
