...
{
    public static class SupportingRolesCalculationInput
    {
        [FunctionName("SupportingRolesCalculationInput")]
            ...
            ILogger log)
        {
            ...
            crmManager.SupportingRolesCalculationInput();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("Upload to SupportingRolesCalculationInput method is successfully executed");
                }
            });
        }
    }
