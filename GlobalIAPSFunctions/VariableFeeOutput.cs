...
{
    public static class VariableFeeOutput
    {
        [FunctionName("VariableFeeOutput")]
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("Upload to VariableFeeOutput method is successfully executed");
                }
            });
        }
    }
