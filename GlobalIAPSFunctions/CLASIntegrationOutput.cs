...
{
    public static class CLASIntegrationOutput
    {
        [FunctionName("CLASIntegrationOutput")]
            ...
            ILogger log)
        {
            ...
            crmManager.RetrieveCLASRecordsFromTable();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation(" Update CLAS Data method is successfully executed");
                }
            });
        }
    }
