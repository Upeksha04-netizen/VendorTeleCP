...
{
    public static class MonthlyAutoEALeadCreation
    {
        [FunctionName("MonthlyAutoEALeadCreation")]
            ...
            ILogger log)
        {
            ...
            crmManager.MonthlyAutoEALeadCreationInput();
        ...
        public static async Task Dispose(ILogger log)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    log.LogInformation("MonthlyAutoEALeadCreation Input function is successfully executed");
                }
            });
        }
    }
